...
# def huggingface_embeddings(para: str) -> list:
#     # 获取模型目录
#     EMBEDDING_MODEL = os.environ.get("HF_EMBEDDING_MODEL")
    
#     # 加载本地模型
#     # embedding_pipeline = pipeline('feature-extraction', model=EMBEDDING_MODEL)    

#     # 使用模型生成嵌入
#     #embeddings = embedding_pipeline(para)
#     embeddings = HuggingFaceEmbeddings(modelname=EMBEDDING_MODEL)

#     # 返回嵌入结果
#     # return embeddings[0].tolist()  # 假设我们只处理一个句子，并且返回第一个句子的嵌入
#     return embeddings
...
def generate_embeddings(cv_paras_map_list:list)->list:
    ...
    for cv_paras_map in  cv_paras_map_list:
        ...
        for para in paras:
            embeddings = huggingface_embeddings(para)
...
def upsert_embeddings(para_embeddings_map_list: list):
    ...
    connection_url = os.environ.get("MONGODB_CONN_URL")
    collection_name = 'gai_embeddings'
    # 创建MongoDB客户端连接，设置SSL相关参数
    # clnt = MongoClient(connection_url, ssl=True, tlsAllowInvalidCertificates=True)
    ...
    try:
        for input_json in para_embeddings_map_list:
            # 移除原input_json中的'_id'字段，避免将其作为更新内容（如果不想更新该字段的话）
            if '_id' in input_json: 
                input_json.pop("_id")
            # 提取用于定位文档的唯一标识（假设是'id'字段，按需替换）
            # filter_criteria = {"file-name": input_json.get("file-name")}
            # 使用合适的更新条件执行upsert操作，更新匹配到的文档或者插入新文档
            # result = collec.update_one(filter_criteria, {'$set': input_json}, upsert=True)
            result = collec.update_one(input_json, {'$set':input_json}, upsert=True)
            # 如果想使用插入逻辑而不是更新插入逻辑，可以启用下面这行（但与方法名upsert不太相符了）
            # result = collec.insert_one(input_json)
    except Exception as err:
        raise err
    finally:
